...
export function ProjectList({ projects }: ProjectListProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {showAll && projects.slice(2).map((project) => <ProjectCard key={project.slug} project={project} />)}
    </div>
  )
}
    ...
    >
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-6 pt-4">
          {project.website && (
            <Link href={project.website} className="text-zinc-400 hover:text-white underline font-mono">
              {project.website}
            </Link>
          )}
          <Link href={`/projects/${project.slug}`} className="text-zinc-400 hover:text-white underline font-mono">
            View Project
          </Link>
        </div>
      </div>
